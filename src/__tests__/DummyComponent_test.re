...
describe(
  "Shallow rendering",
  () => {
    let setup = (~title="Test", ()) => Enzyme.shallow(<DummyComponent title />);
    let header = (wrapper) => wrapper |> Enzyme.Shallow.find("#header") |> Enzyme.Shallow.first;
    ...
    describe(
      ...
      () => {
        ...
        test(
          "has the expected title in the #header",
          () => {
            let title = "A test title";
            let wrapper = setup(~title, ());
        ...
        test(
          ...
          () => {
            ...
            let expectedNode = <div id="header"> <h1> (ReasonReact.string(title)) </h1> </div>;
          ...
          () => {
            ...
            let result =
              Enzyme.Shallow.foldLeft(
                (text, node) => text ++ Enzyme.Shallow.text(node),
                "",
                items
              );
        ...
        test(
          "folds right properly",
          () => {
            let items = setup() |> listItems;
            let result =
              Enzyme.Shallow.foldRight(
                (text, node) => text ++ Enzyme.Shallow.text(node),
                "",
                items
              );
...
describe(
  "Full DOM rendering",
  () => {
    let setup = (~title="Test", ()) => Enzyme.mount(<DummyComponent title />);
    let header = (wrapper) => wrapper |> Enzyme.Mount.find("#header") |> Enzyme.Mount.first;
    let listItems = (wrapper) => wrapper |> Enzyme.Mount.find("#list") |> Enzyme.Mount.children;
    describe(
      "DummyComponent",
      () => {
        open Expect;
        ...
        test(
          ...
          () => {
            let items = setup() |> listItems;
            let result =
              Enzyme.Mount.foldLeft((text, node) => text ++ Enzyme.Mount.text(node), "", items);
            expect(result) |> toBe("OneTwoThree")
          }
        );
        test(
          ...
          () => {
            let items = setup() |> listItems;
            let result =
              Enzyme.Mount.foldRight((text, node) => text ++ Enzyme.Mount.text(node), "", items);
            expect(result) |> toBe("ThreeTwoOne")
          }
        )
      }
